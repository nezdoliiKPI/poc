...
public class AirDataGenerator extends DeviceDataGenerator{
    ...
    public AirDataGenerator(
        ...
        final var mainTiming = new MessageTiming(TimeUnit.MINUTES, 0, 1, 1);
    ...
    public Object getData() {
        final var co2 = 350 + random.nextInt(600);
        final var pm25 = random.nextFloat() * 15.0f;
        final var pm10 = random.nextFloat() * 30.0f;
        final var tvoc = random.nextFloat() * 0.5f;
        final var t = 25.0f + (random.nextFloat() * 5.0f);
        final var h = 45.0f + (random.nextFloat() * 20.0f);
